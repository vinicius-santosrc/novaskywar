package br.dev.santos.skywar.commands;
...
public class ResetWorldCommand implements CommandExecutor {
    ...
    public boolean resetArena(Arena arena) {
        World world = Bukkit.getWorld(arena.getNameOfTheWorld());

        Bukkit.unloadWorld(world, false);
        Bukkit.getLogger().info("Mundo descarregado: " + arena.getNameOfTheWorld());

        File worldFolder = world.getWorldFolder();
        File backupFolder = new File(worldBackupFolder, arena.getNameOfTheWorld());
        ...
        Bukkit.createWorld(new WorldCreator(arena.getNameOfTheWorld()));
        Bukkit.getLogger().info("Arena: " + arena.getNameOfTheWorld() + " foi restaurada.");
